...
public class RentCompanyEmbadded extends AbstractRentCompany implements Persistable, IRentCompany {
    ...
    // sprint 2
    Map<String, List<Car>> modelCars = new HashMap<>(); //model name -> list cars
    ...
    @Override
    public int getFinePercent(int finePercent) {
        return 0;
    }
    ...
    public CarsReturnCode addDriver(Driver driver) {
        return drivers.putIfAbsent(driver.getLisenseId(), driver) == null ? OK : DRIVER_EXISTS;
    }
...
//        records.putIfAbsent(rentDate, listRecords);
    ...
    public List<Car> getCarsByModel(String modelName) {
        ...
        return res.stream().filter(c -> !c.isFlRemove() && !c.isInUse()).toList();
    }
    ...
    public RemoveCarData removeCar(String regNumber) {
        ...
        return car.isInUse() ? new RemoveCarData(car, null) : actualCarRemove(car);
    }
    ...
    public List<RemoveCarData> removeModel(String modelName) {
        ...
        return carsModel.stream().filter(c -> !c.isFlRemove()).map(c -> removeCar(c.getRegNumber()))
                // .filter(Objects::nonNull)
                .toList();
    }

    @Override
    public RemoveCarData returnCar(String regNumber, long licenseId, LocalDate returnDate, int damages,
                                    int tankPercent) {
        ...
        if (damages > REMOVE_THRESHOLD || car.isFlRemove()) {
            car.setFlRemove(true);
            RemoveCarData remCarData = actualCarRemove(car);
            return remCarData != null ? remCarData :
                    new RemoveCarData(car, new ArrayList<>());
        }
    ...
    private void updateRecord(RentRecord record, LocalDate returnDate, int damages, int tankPercent) {
        ...
        record.setTankPersent(tankPercent);
        record.setReturnDate(returnDate);

        double cost = computeCost(getRentPricePerDay(record.getRegNumber()),
                record.getRentDays(),
                getDaysDelay(record), tankPercent, getTankVolume(record.getRegNumber()));
    ...
    public static IRentCompany restoreFromFile (String fileName) throws IOException, ClassNotFoundException {
        ...
        return new RentCompanyEmbadded();
    }
}
